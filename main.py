...
# ============================================================
# --diagnose: 최종 gbest 해 하나에 대해서만 사후 페널티 분해 (전 입자 기록은 안 함)
# ============================================================

def _print_diagnosis(n_ess, run_idx, gbest_x_3n):
    """부록B의 LAMBDA_V/LAMBDA_LINE/PENALTY_DIVERGE는 전부 "잠정값, 첫 실행 로그 보고
    조정"이므로 gbest 해의 페널티 성분을 볼 수단이 필요하다. evaluate.evaluate_particle의
    return_detail=True가 이미 v_violation/i_violation(가중 전 성분)을 반환하므로 evaluate.py를
    고치지 않고 params의 LAMBDA_V/LAMBDA_LINE을 곱해 가중 페널티를 사후 재계산한다."""
    x4n = _expand_to_4n(gbest_x_3n, n_ess)
    detail = evaluate.evaluate_particle(x4n, return_detail=True)

    print(f'  [진단] n_ess={n_ess} run={run_idx} gbest 해 페널티 분해:', flush=True)
    if detail.get('diverged'):
        print('    gbest 해가 발산 상태입니다 - 페널티 분해 불가(정상 평가가 아님).', flush=True)
        return

    penalty_v = PM.LAMBDA_V * detail['v_violation']
    penalty_line = PM.LAMBDA_LINE * detail['i_violation']
    print(f"    j_net={detail['j_net']:.4e}  b_energy={detail['b_energy']:.4e}  "
          f"b_defer={detail['b_defer']:.4e}  cost={detail['cost']:.4e}", flush=True)
    print(f"    v_violation={detail['v_violation']:.6f} pu  -> penalty_v={penalty_v:.4e} 원", flush=True)
    print(f"    i_violation={detail['i_violation']:.6f}     -> penalty_line={penalty_line:.4e} 원", flush=True)
    print(f"    fitness(재계산 검산)={detail['fitness']:.6e}", flush=True)
...
def run_for_n_ess(n_ess, profile, n_workers, base_seed, diagnose):
    ...
    try:
        for run_idx in range(profile['n_runs']):
            ...
            best_gbest_f = min(best_gbest_f, float(result['f']))
            ...
            if diagnose:
                _print_diagnosis(n_ess, run_idx, result['x'])
    finally:
        pool.close()
        pool.join()
