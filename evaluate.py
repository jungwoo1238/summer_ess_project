...
def _solve_unit_schedules(b, S, E, base_p_sum):
    """ALL_DAYS 전 시나리오에 대해 스케줄 P_net[s](n,T)/Q[s](n,T)를 조인트로 푼다.
    AVG_DAYS는 solve_avg(조달비 대리), PEAK_DAYS는 solve_peak(시스템 전체 피크 대리,
    통합 - 2절/7-A절 "기수 축 d* 중복적용" 결함 해소) - CLAUDE.md 2절.

    S_i<=0/E_i<=0인 기도 별도 분기 없이 그대로 LP에 들어간다(lower_lp가 내부적으로
    자기방전 상쇄용 최소 바닥만 깔고 나머지는 자연히 0에 수렴 - lower_lp._s_floor_for_
    self_discharge 참조, 예전의 "S<=0/E<=0이면 LP 미호출 후 0 반환" 특수분기가 조인트
    구조에서는 성립하지 않는다: 한 기가 infeasible해지면 시스템 전체가 infeasible해지므로
    lower_lp 쪽에서 근본적으로 처리한다).

    본실험 속도를 위해 assert_physics=False로 호출한다(물리보존 검증은 lower_lp 자체 테스트에서
    이미 끝남 - CLAUDE.md 7절 LP검증 항목은 test_lp.py가 담당).

    반환: (unit_p, unit_q) 각각 dict[scenario] -> (n, T) ndarray.
    """
    unit_p = {}
    unit_q = {}
    ...
    for s in PM.AVG_DAYS:
        ...
        P_net, Q, _soc = solve_avg(S, E, b, smp, profile, assert_physics=False)
        unit_p[s] = P_net
        unit_q[s] = Q
    ...
    for s in PM.PEAK_DAYS:
        ...
        P_net, Q, _soc, _pk = solve_peak(S, E, b, load_total, profile, assert_physics=False)
        unit_p[s] = P_net
        unit_q[s] = Q
...
def evaluate_particle(x, return_detail=False):
    ...
    # PCS(인버터) 무효전력 변환손실 (CMD_pcs_loss.md) - Q=0이면 정확히 0(회귀 안전).
    # 아래 주입 루프에서 sgen 유효전력을 이만큼 낮추고, loss_ess에 이만큼 더한다
    # (인버터가 자기 손실을 스스로 부담 -> 계통에 나가는 순 유효전력이 감소 - 3-(c) 참조).
    ...
    for s in PM.ALL_DAYS:
        ...
        for t in range(PM.TIME_STEPS):
            ...
            net.load['q_mvar'] = base_q * scale
            for i in range(n):
                # ★ 계통에 나가는 순 유효전력 = LP가 정한 P - PCS 무효전력 손실(인버터가
                # 자기 손실을 스스로 부담). Q는 그대로 주입(손실은 P 쪽에서만 흡수).
            ...
            # ★ 계통 선로손실(pandapower) + PCS 손실을 더한다. 슬랙 수지 항등식
            # (P_slack=SigmaLoad+Loss-P_ESS)에서 P_ESS를 "LP 원본"(unit_p, 감산 전)으로
            # 유지하는 대신 Loss 쪽에 PCS분을 더해야 항등식이 대수적으로 정확히 성립한다
            # (주입을 P-Loss_pcs로 낮췄으므로: Loss_line-(P-Loss_pcs) = (Loss_line+Loss_pcs)-P
            # - 아래 p_ess_total은 원본 unit_p 그대로 쓴다).
    ...
    # ★ p_ess_total은 unit_p(LP 원본, PCS손실 감산 전)로 유지한다 - loss_ess가 이미
    # +unit_loss_pcs로 보정됐으므로 여기서 또 빼면 이중차감이 된다(위 주입 루프 주석 참조).
